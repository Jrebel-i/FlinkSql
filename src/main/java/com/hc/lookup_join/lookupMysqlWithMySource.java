...
public class lookupMysqlWithMySource {
...
//        conf.setString("execution.savepoint.path", "file:///Users/flink/checkpoints/ce2e1969c5088bf27daf35d4907659fd/chk-5");
        ...
        //流表 只产生redis中存在的user_id的流数据  比如redis中只有a,b；那么就只生成a,b用户
        //不知道为什么这个lookup有点问题，如果是关联不到就会报错，或者直接就所有的都关联不上
        DataStreamSource<Row> rowDataStreamSource = env.addSource(new UserDefinedSource());
//        tenv.createTemporaryView("show_log",rowDataStreamSource);
        ...
        tenv.executeSql("CREATE TABLE user_profile (\n" +
                ...
                "    ) WITH (\n" +
                "  'connector' = 'jdbc',\n" +
                "  'url' = 'jdbc:mysql://localhost:3306/s3_data?serverTimezone=UTC',\n" +
